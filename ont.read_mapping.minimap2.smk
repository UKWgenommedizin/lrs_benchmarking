...
OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".mm2-ont.cram", zip, dataset=DATASETS)

#OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".mm2-ont.cram.md5", zip, dataset=DATASETS)

OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".mm2-ont.cram.idxstats", zip, dataset=DATASETS)

#OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + "mm2-ont..cram.flagstat", zip, dataset=DATASETS)

OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".mm2-ont.cram.stats", zip, dataset=DATASETS)
...
ruleorder: minimap2_ont_unmapped_bam > minimap2_ont_fastq


rule minimap2_ont_fastq:
	...
	output:	cram="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram"
	params:	prefix="{cwd}/cram/tmp/{dataset}.{genome_build}.mm2-ont"
	log:	"{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.log"
...
rule minimap2_ont_unmapped_bam:
	...
	shell:	"umask 0027; \
		...
		srun -p all -c {threads} --mem={resources.mem_gb}GB /bin/bash -c \" \
			...
			\\$VARCAD_PATH/bin/samtools fastq \
				-T ch,du,dx,f5,ML,MM,mx,ns,pi,qs,RG,rn,sd,sm,st,sv,ts \
				{input.unmapped_bam} | \
			/mnt/storage/groups/heinz/tools/minimap2-2.28_x64-linux/minimap2 \
				-t {threads} \
				-R '@RG\\tID:{wildcards.dataset}\\tSM:{wildcards.dataset}' \
...
rule md5sum_cram:
	input:	cram="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram"
	output:	md5="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.md5"
	log:	"{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.md5.log"
	message:"executing {rule} with output {output} and input {input}"
	threads:1
	resources:
		mem_gb=2
...
rule samtools_idxstats:
	input:	cram="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram"
	output:	idxstats="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.idxstats"
	log:	"{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.idxstats.log"
	message:"executing {rule} with output {output} and input {input}"
	threads:2
	resources:
		mem_gb=4
...
rule samtools_flagstat:
	input:	cram="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram"
	output:	flagstat="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.flagstat"
	log:	"{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.flagstat.log"
	message:"executing {rule} with output {output} and input {input}"
	threads:2
	resources:
		mem_gb=4
...
rule samtools_stats:
	input:	cram="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram",
		fasta=VARCAD_DB_PATH + "/{genome_build}/Reference_sequence/" + VARCAD_DB_VERSION + "/{genome_build}.fasta.gz"
	output:	stats="{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.stats"
	log:	"{cwd}/cram/{dataset}.{genome_build}.mm2-ont.cram.stats.log"
	message:"executing {rule} with output {output} and input {input}"
	threads:16
	resources:
		mem_gb=32
