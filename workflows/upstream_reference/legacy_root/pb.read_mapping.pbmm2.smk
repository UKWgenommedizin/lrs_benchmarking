...
OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".pbmm2-pb.cram", zip, dataset=DATASETS)

#OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".pbmm2-pb.cram.md5", zip, dataset=DATASETS)

OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".pbmm2-pb.cram.idxstats", zip, dataset=DATASETS)

#OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".pbmm2-pb.cram.flagstat", zip, dataset=DATASETS)

OUTPUT = OUTPUT + expand(CWD + "/cram/{dataset}." + VARCAD_GENOME_BUILD + ".pbmm2-pb.cram.stats", zip, dataset=DATASETS)
...
ruleorder: pbmm2_pb_unmapped_bam > pbmm2_pb_fastq
...
rule pbmm2_pb_fastq:
	...
	output:	cram="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram"
	params:	prefix="{cwd}/cram/tmp/{dataset}.{genome_build}.pbmm2-pb"
	log:	"{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.log"
...
rule md5sum_cram:
	input:	cram="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram"
	output:	md5="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.md5"
	log:	"{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.md5.log"
	message:"executing {rule} with output {output} and input {input}"
	threads:1
	resources:
		mem_gb=2
...
rule samtools_idxstats:
	input:	cram="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram"
	output:	idxstats="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.idxstats"
	log:	"{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.idxstats.log"
	message:"executing {rule} with output {output} and input {input}"
	threads:2
	resources:
		mem_gb=4
...
rule samtools_flagstat:
	input:	cram="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram"
	output:	flagstat="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.flagstat"
	log:	"{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.flagstat.log"
	message:"executing {rule} with output {output} and input {input}"
	threads:2
	resources:
		mem_gb=4
...
rule samtools_stats:
	input:	cram="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram",
		fasta=VARCAD_DB_PATH + "/{genome_build}/Reference_sequence/" + VARCAD_DB_VERSION + "/{genome_build}.fasta.gz"
	output:	stats="{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.stats"
	log:	"{cwd}/cram/{dataset}.{genome_build}.pbmm2-pb.cram.stats.log"
	message:"executing {rule} with output {output} and input {input}"
	threads:16
	resources:
		mem_gb=32
