...
OUTPUT = OUTPUT + expand(CWD + "/vcf_called/snv_indel/tmp/{dataset}.hg38.{mapper}.clair3-ont/merge_output.vcf.gz", zip, dataset=DATASETS, mapper=MAPPERS)

OUTPUT = OUTPUT + expand(CWD + "/vcf_called/snv_indel/{dataset}.hg38.{mapper}.clair3-ont.vcf.gz", zip, dataset=DATASETS, mapper=MAPPERS)

OUTPUT = OUTPUT + expand(CWD + "/vcf_called/snv_indel/{dataset}.hg38.{mapper}.clair3-ont.vcf.gz.stats", zip, dataset=DATASETS, mapper=MAPPERS)
...
rule run_clair3_ont:
    ...
    output:  vcf_gz="{cwd}/vcf_called/snv_indel/tmp/{dataset}.hg38.{mapper}.clair3-ont/merge_output.vcf.gz"
    params:  output_dir="{cwd}/vcf_called/snv_indel/tmp/{dataset}.hg38.{mapper}.clair3-ont"
    message: "executing {rule} with output {output} and input {input}"
    log:     "{cwd}/vcf_called/snv_indel/{dataset}.hg38.{mapper}.clair3-ont.log"
    threads: 64
    resources:
	    mem_gb=160
    shell:   "umask 0027; \
...
rule copy_clair3_vcf_gz:
    input:   vcf_gz="{cwd}/vcf_called/snv_indel/tmp/{dataset}.hg38.{mapper}.clair3-ont/merge_output.vcf.gz"
    output:  vcf_gz="{cwd}/vcf_called/snv_indel/{dataset}.hg38.{mapper}.clair3-ont.vcf.gz"
    message: "executing {rule} with {output} and input {input}"
    log:     "{cwd}/vcf_called/snv_indel/{dataset}.hg38.{mapper}.clair3-ont.vcf.gz.log"
    threads: 2
    resources:
	    mem_gb=4
    shell:   "umask 0027; \
...
rule bcftools_stats:
    input:   vcf_gz="{cwd}/vcf_called/snv_indel/{dataset}.hg38.{mapper}.clair3-ont.vcf.gz", fasta=FASTA_PATH
    output:  stats="{cwd}/vcf_called/snv_indel/{dataset}.hg38.{mapper}.clair3-ont.vcf.gz.stats"
    message: "executing {rule} with {output} and input {input}"
    threads: 2
    resources:
	    mem_gb=4
