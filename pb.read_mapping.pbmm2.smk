...
# Read mapping workflow for PacBio HiFi data using pbmm2.
# mapper_tag: pbmm2-pb
# Benchmark note: HIFI is the pbmm2 preset used for PacBio HiFi reads.
...
DATASETS = [d for d in DATASETS if ".pb." in d.lower()]
...
rule pbmm2_pb_map_sort:
    ...
    shell:
        ...
        (
            ...
            echo "[$(date -Is)] START pbmm2_pb_map_sort {wildcards.dataset}"
            echo "Dataset: {wildcards.dataset}"
            echo "Reference: {input.ref}"
            echo "Mapper: pbmm2 {PBMM2_VERSION}"
            echo "Preset: {PBMM2_PRESET}"
            echo "Mapper container hostname: pbmm2-pb-{wildcards.dataset}"
            ...
            docker run --rm \
                --hostname "pbmm2-pb-{wildcards.dataset}" \
            ...
            echo "[$(date -Is)] END pbmm2_pb_map_sort {wildcards.dataset}"
        ) > "{log}" 2>&1
        """
...
rule pbmm2_pb_idxstats:
    ...
    shell:
        ...
        (
            ...
            echo "[$(date -Is)] START pbmm2_pb_idxstats {wildcards.dataset}"
            ...
            echo "[$(date -Is)] END pbmm2_pb_idxstats {wildcards.dataset}"
        ) > "{log}" 2>&1
        """
...
rule pbmm2_pb_stats:
    ...
    shell:
        ...
        (
            ...
            echo "[$(date -Is)] START pbmm2_pb_stats {wildcards.dataset}"
            ...
            echo "[$(date -Is)] END pbmm2_pb_stats {wildcards.dataset}"
        ) > "{log}" 2>&1
        """
