...
# Read mapping workflow for PacBio HiFi data using minimap2.
# mapper_tag: mm2-pb
...
rule minimap2_pb_map_sort:
    ...
    shell:
        ...
        (
            ...
            echo "[$(date -Is)] START minimap2_pb_map_sort {wildcards.dataset}"
            echo "Dataset: {wildcards.dataset}"
            echo "Reference: {input.ref}"
            echo "Mapper: minimap2 {MINIMAP2_VERSION}"
            echo "Preset: map-hifi"
            echo "Mapper container hostname: mm2-pb-{wildcards.dataset}"
            ...
            docker run --rm \
                --hostname "mm2-pb-{wildcards.dataset}" \
            ...
            echo "[$(date -Is)] END minimap2_pb_map_sort {wildcards.dataset}"
        ) > "{log}" 2>&1
        """
...
rule minimap2_pb_idxstats:
    ...
    shell:
        ...
        (
            ...
            echo "[$(date -Is)] START minimap2_pb_idxstats {wildcards.dataset}"
            ...
            echo "[$(date -Is)] END minimap2_pb_idxstats {wildcards.dataset}"
        ) > "{log}" 2>&1
        """
...
rule minimap2_pb_stats:
    ...
    shell:
        ...
        (
            ...
            echo "[$(date -Is)] START minimap2_pb_stats {wildcards.dataset}"
            ...
            echo "[$(date -Is)] END minimap2_pb_stats {wildcards.dataset}"
        ) > "{log}" 2>&1
        """
